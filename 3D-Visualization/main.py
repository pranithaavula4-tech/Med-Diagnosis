...
ORIGINAL_SLICES_FOLDER = STATIC_DIR / "original_slices"
...
for folder in [
    ...
    OUTPUT_MASKS_FOLDER,
    ORIGINAL_NII_FOLDER,
    PREDICTED_NII_FOLDER,
    MASK_IMAGES_FOLDER,
    ORIGINAL_SLICES_FOLDER
]:
...
async def upload_nii_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        for i in range(depth):
            ...
            # Save for processing
            temp_slice_path = SLICES_FOLDER / f"slice_{i:03d}.png"
            cv2.imwrite(str(temp_slice_path), slice_rgb)

            # Save for frontend display
            display_slice_path = ORIGINAL_SLICES_FOLDER / f"slice_{i:03d}.png"
            cv2.imwrite(str(display_slice_path), slice_rgb)
        ...
        for image_name in image_files:
            ...
            success = cv2.imwrite(str(output_mask_path), mask_thresh)
            print("Saving mask:", output_mask_path)
            print("Save success:", success)
            print("Mask shape:", mask_thresh.shape)
        ...
        slice_urls = [
            f"/static/original_slices/slice_{i:03d}.png"
            for i in range(depth)
        ]
        return {
            "success": True,
